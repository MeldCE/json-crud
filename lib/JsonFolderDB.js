...
module.exports = function JsonFolderDB(file, options) {
  ...
  function saveData(id, data) {
    ...
    if (typeof data === 'undefined') {
      ...
      return writeFile(filename, JSON.stringify(data, null, 2));/*XXX.then(function() {
        if (options.cacheValues) {
          cachedData[id] = data;
        }
        if (options.cacheKeys && cachedKeys.indexOf(id) === -1) {
          cachedKeys.push(id);
        }
      };*/
    }
  ...
  function doDelete(filter) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (common.keyTypes.indexOf(typeof filter) !== -1) {
        ...
        /*XXX if (options.cacheValues) {
          console.log('have cached data', cachedData);
          if (filter === true) {
            deletedKeys = Object.keys(cachedData);
            Object.keys(cachedData).forEach(function(key) {
              console.log('adding job to delete', key);
              deletes.push(saveData(key));
            });
          } else {
          ...
          keysPromise.then(function(existingKeys) {
            console.log('non-cached data existing keys are', existingKeys);
            if (filter === true) {
              deletedKeys = existingKeys;
              existingKeys.forEach(function(key) {
                deletes.push(saveData(key));
